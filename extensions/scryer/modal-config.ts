...
function cleanRow(value: unknown): number | undefined {
	const n = Number(value);
	return Number.isFinite(n) && n >= 0 ? Math.round(n) : undefined;
}
...
export function modalAnchorOption(config: ModalConfig): "center" | "top-center" {
	return config.top === undefined ? "center" : "top-center";
}

export function modalOffsetYOption(config: ModalConfig): number {
	return config.top ?? 0;
}
...
export function describeModalConfig(config: ModalConfig): string {
	return `width=${config.width ? `${config.width} cols` : "90%"}, height=${config.height ? `${config.height} rows` : "80%"}, top=${config.top === undefined ? "center" : `${config.top} rows`}`;
}
...
export function parseModalConfigArgs(args: string, existing: ModalConfig): { config?: ModalConfig; message: string; showOnly?: boolean } {
	...
	if (/^(reset|default|defaults)$/i.test(trimmed)) return { config: {}, message: "modal config reset to width=90%, height=80%, top=center" };
	...
	for (let i = 0; i < tokens.length; i++) {
		...
		const assignment = token.match(/^(width|w|cols?|columns?|height|h|rows?|top|y|row)=(\d+)$/i);
		if (assignment) {
			const key = assignment[1].toLowerCase();
			const value = Number(assignment[2]);
			if (/^(width|w|col|cols|columns?)$/.test(key)) next.width = value;
			else if (/^(height|h|row|rows?)$/.test(key)) next.height = value;
			else next.top = value;
			continue;
		}
		if (/^(width|w|cols?|columns?|height|h|rows?|top|y|row)$/i.test(token) && tokens[i + 1]) {
			...
			else if (/^(height|h|rows?)$/i.test(token)) next.height = value;
			else next.top = value;
			continue;
		}
		const value = Number(token);
		if (Number.isFinite(value)) positional.push(value);
		else return { message: `usage: /modal-config [width <cols>] [height <rows>] [top <rows>] | /modal-config <cols> <rows> [top] | /modal-config reset` };
	}

	if (positional.length > 0) next.width = positional[0];
	if (positional.length > 1) next.height = positional[1];
	if (positional.length > 2) next.top = positional[2];
	if (positional.length > 3) return { message: "too many positional values; use /modal-config <cols> <rows> <top>" };

	next = { width: cleanNumber(next.width), height: cleanNumber(next.height), top: cleanRow(next.top) };
